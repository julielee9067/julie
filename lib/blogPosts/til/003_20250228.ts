...
export const til20250228: BlogPost = {
    ...
    date: "2025-02-28 17:26",
    category: {
        ko: "TIL",
        en: "TIL",
    },
    description: {
        ko: "MongoDB 전체 텍스트 검색을 사용할 때의 임베디드 문서 관련 제한 사항",
...
근데 \`"type": "a"\`, \`"value": "something"\` 처럼 구체적인 필드 조합으로 검색하려면 embedded document index가 필요하다. ([**참조**](https://www.mongodb.com/docs/atlas/atlas-search/embedded-document/))

문제는 embedded document index를 생성하면 array 안의 각 객체마다 하나의 문서가 인덱싱된다는 점이었다.

MognoDB Atlas에서는 샤드 하나당 최대 21억(2.1 billion)개의 embedded 문서를 인덱싱할 수 있다.
...
2. **Full text search 대신 element search**
    ...
    근데 이 방법은 **데이터들이 매우 일관적(standardized)으로 저장되어 있어야 동작**한다는 장점이 있다.
...
3. **샤딩**
    ...
    그리고 비용이 더 많이 든다. ([**참조**](https://www.mongodb.com/resources/products/capabilities/database-sharding-explained#:~:text=Sharding%20does%20come%20with%20several,operation%20to%20the%20appropriate%20shard.))
