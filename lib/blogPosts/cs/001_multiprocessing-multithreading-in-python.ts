...
export const csMultiprocessingAndMultiThreadingInPython: BlogPost = {
    ...
    description: {
        ko: "멀티 프로세싱과 멀티 스레딩의 파이썬에서의 동작 원리",
...
#### 동시에 실행할 수 있는 스레드의 수
멀티 스레딩에서 **동시에 실행할 수 있는 스레드의 수**도 알아봅시다.

**CPU bound** 작업은 한 번에 한 스레드만 실행되기 때문에, 보통 CPU 코어 수에 비슷하게 맞추는게 좋습니다. CPython에서는 GIL 때문에 스레드의 수를 코어 수에 맞추더라도, **여전히 한 번에 한 스레드만 Python bytecode를 실행**합니다. 대신, 불필요한 스레드 전환 관리 오버헤드를 피할 수 있습니다.\n
대신 **I/O bound** 작업(대기 시간이 긴 작업)은 **CPU 코어 수보다 훨씬 많은 스레드를 사용**할 수 있습니다. 예를 들어, 네트워크 요청이 많은 애플리케이션에서는 수십 개 또는 그 이상의 스레드를 생성해도 성능 향상이 가능할 수 있습니다.
