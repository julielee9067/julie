...
이 방법은 엄밀히 말해 **실시간**은 아니지만, 개념이 간단하고 구현하기도 쉽습니다.

1. 클라이언트는 **주기적으로** HTTP 요청을 보내 새 데이터가 있는지 확인합니다.
2. 서버는 해당 요청을 받을 때마다 DB에서 **아직 읽지 않은 메시지**를 조회해 응답합니다.

이 방법의 장점은 구현이 쉽고, 특별한 인프라 없이도 가능하며, 서버 입장에서 상태 관리를 할 필요가 거의 없다는 점입니다.
...
- **더 빠른 실시간**: 롱 폴링, SSE  
- **양방향 통신** 필요할 때: WebSocket  
- **화상/음성 통화, P2P 구조** 필요할 때: WebRTC
